...
function [T,sol] = NBarLink_DAE(n,rederive,constrain,simulate,p,Y0,time)
...
if rederive == 1
    ...
    % Constraint Forces
    if constrain == 1
        Fx      = sym('Fx',[1 n+1], 'real');
        Fy      = sym('Fy',[1 n+1], 'real');
    else
        Fx      = sym('Fx',[1 n], 'real');
        Fy      = sym('Fy',[1 n], 'real');
    end
    ...
    if constrain == 1
        lmb{n}    = m(n)*aGrelOCartesian{n}-(-F{n}+F{n+1})-Fg{n};
    ...
    amb{1}    = I(1)*ddt(1)*e3 - (cross(-rGrelO{1},F{1})+cross(rErelO{1}-rGrelO{1},F{2}));
    eq{2*n+1} = dot(amb{1},e3);
    for i = 2:n-1
        amb{i} = I(i)*ddt(i)*e3 - (cross(rErelO{i-1}-rGrelO{i},-F{i})+cross(rErelO{i}-rGrelO{i},F{i+1}));
        eq{2*n+i} = dot(amb{i},e3);
    end
    if constrain == 1
        amb{n}  = I(n)*ddt(n)*e3 - (cross(rErelO{n-1}-rGrelO{n},-F{n})+cross(rErelO{n}-rGrelO{n},F{n+1}));
        eq{3*n} = dot(amb{n},e3);
    else
        amb{n}  = I(n)*ddt(n)*e3 - (cross(rErelO{n-1}-rGrelO{n},-F{n}));
        eq{3*n} = dot(amb{n},e3);
    end
    ...
    if constrain == 1
        eq{5*n+1} = dot(aErelO{n},e1);
        eq{5*n+2} = dot(aErelO{n},e2);
    end
    
    [A,b] = equationsToMatrix(eq,[ddt,ddx,ddy,Fx,Fy]);
    
    matlabFunction(A,'file','NBarLink_A_DAE','Vars',{g,a,L,m,t,dt});
    matlabFunction(b,'file','NBarLink_b_DAE','Vars',{g,a,L,m,t,dt});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function dydt = odeNBarDAE(~,y,p)
        
        th   = y(1:p.n)';
        dth  = y(p.n+1:2*p.n)';
        P    = NBarLink_A_DAE(p.g,p.a,p.L,p.m,th,dth);
        q    = NBarLink_b_DAE(p.g,p.a,p.L,p.m,th,dth);
        x    = P\q;
        
        dydt = [dth, x(1:p.n)']';
    end

tVector = linspace(time(1),time(end),(time(end)-time(1))*50);

opt.relTol = 1e-10;
opt.absTol = 1e-10;

% Integration
[T,sol] = ode45(@odeNBarDAE,tVector,Y0,opt,p);
...
if simulate == 1
    ...
    if constrain == 1
        miny = -max(abs(maxx),abs(minx));
        maxy =  max(abs(maxx),abs(minx));
    end 
    ...
    if constrain == 1
        conBar = plot([0 xE(idx,end)],[0 yE(idx,end)],'Color','r','LineWidth',1);
    end
    ...
    while idx < length(tVector)
        ...
        if constrain == 1
            set(conBar,'XData',[0 xE(idx,end)],'YData',[0 yE(idx,end)])
        end
